...
static void RenderRageTab() {
    ...
    UI::BeginCard("Other");
    {
        ...
        UI::Checkbox("No Spread",        &Globals::rage_no_spread);
...
static void RenderLegitTab() {
    ...
    UI::ColTitle("Accuracy");
    UI::BeginCard("No Spread", badge);
    {
        UI::Checkbox("No Spread", &Globals::legit_no_spread);
    }
    UI::EndCard();
    ...
    UI::BeginCard("RCS", badge);
    {
        ...
        if (prof.rcs_enabled || IsBound("Enable RCS")) {
            static const char* rcsModes[] = { "Legit (camera pull)", "Silent (no shake)" };
            int rcsMode = prof.rcs_silent ? 1 : 0;
            if (UI::Combo("Mode", &rcsMode, rcsModes, 2)) prof.rcs_silent = (rcsMode == 1);

...
// CS2 player skeleton bone indices. Restored to the Head=6 layout that worked
// in-game (commit 01744e8 had shifted these to Head=7 and broke aim/ESP). The
// extra names the rage/legit code references (Chest, Spine2/1, clavicles, elbows)
// are aliased onto the nearest real bone in this scheme — C++ allows duplicate
// enumerator values, and aiming at the aliased bone hits the correct body region.
enum class BoneID : int {
    Head          = 6,
    Neck          = 5,
    Spine2        = 4,   // upper spine
    Chest         = 4,   // alias → upper spine (chest hitbox region)
    Spine1        = 4,   // alias → upper spine
    Pelvis        = 0,
    LeftClavicle  = 8,   // alias → left shoulder
    LeftShoulder  = 8,
    LeftElbow     = 9,   // (old LeftArm)
    LeftHand      = 11,
    RightClavicle = 13,  // alias → right shoulder
    RightShoulder = 13,
    RightElbow    = 14,  // (old RightArm)
    RightHand     = 16,
    LeftHip       = 22,
    LeftKnee      = 23,
    LeftFoot      = 24,
...
namespace Bones
{
    inline const std::vector<BoneConnection> connections =
    {
        ...
        { BoneID::Neck,   BoneID::Chest  },
        { BoneID::Chest,  BoneID::Spine2 },
        { BoneID::Spine2, BoneID::Spine1 },
        { BoneID::Spine1, BoneID::Pelvis },

        // Left arm
        { BoneID::Chest,         BoneID::LeftClavicle  },
        { BoneID::LeftClavicle,  BoneID::LeftShoulder  },
        { BoneID::LeftShoulder,  BoneID::LeftElbow     },
        { BoneID::LeftElbow,     BoneID::LeftHand      },

        // Right arm
        { BoneID::Chest,          BoneID::RightClavicle  },
        { BoneID::RightClavicle,  BoneID::RightShoulder  },
...
namespace Utils
{
    ...
    // SEH-safe bone read. Returns false (and leaves out untouched) if any
    // dereference faults — the entity list can hold stale pawn pointers whose
    // scene node / bone array is being torn down on another thread.
    inline bool SafeReadBonePos(C_CSPlayerPawn* pawn, BoneID boneID, Vector& out)
    {
        __try
        {
            if (!pawn)
            ...
            if (!IsValidPtr(reinterpret_cast<uintptr_t>(scene)))
                return false;

            uintptr_t boneArray = *reinterpret_cast<uintptr_t*>(
                reinterpret_cast<uintptr_t>(scene) + Offsets::m_modelState + 0x80);
            if (!IsValidPtr(boneArray))
                return false;

            out = *reinterpret_cast<Vector*>(boneArray + static_cast<int>(boneID) * 0x20);
            return true;
        }

...
namespace {
    ...
    static uintptr_t ResolveFromList(uintptr_t listBase, uint32_t handle) {
        // Invalid handle (0xFFFFFFFF / -1 index) — bail before it masks down to a
        // garbage slot (0x7FFF) and yields a bogus pointer that callers then
        // dereference. This was the FSN0:Aimbot "AV read of 0xFFFF...FF" crash on
        // team-select / respawn.
        if (handle == 0xFFFFFFFFu) return 0;
        uint32_t index = handle & 0x7FFF;
        if (index >= 0x7FF0) return 0;   // reserved/invalid index range

        uintptr_t entry = 0;
        if (!SafeReadPtr(listBase + 8 * (index >> 9) + 16, entry) || !entry)
            return 0;
        uintptr_t ptr = 0;
        if (!SafeReadPtr(entry + 112 * (index & 0x1FF), ptr))
            return 0;
        // Range-check the resolved pointer so a corrupt slot can't return -1 etc.
...
void EntityManager::Update()
{
    ...
    // End of match / disconnect / map change: there is no local pawn. The entity
    // list is being torn down, so handles resolve to freed-but-in-range pointers
    // that fault on every read. Those faults are caught by SEH but the VEH still
    // logs each one — flooding the log/file I/O every frame and tanking FPS until
    // it looks like a hang. Bail out cleanly and publish an empty snapshot so all
    // downstream consumers (Visuals removals, silent aim, ESP) iterate nothing.
    if (!currentLocalPawn) {
...
{
    std::shared_lock lock(mutex);
    return entities;   // copied under the lock — safe to iterate after release
}

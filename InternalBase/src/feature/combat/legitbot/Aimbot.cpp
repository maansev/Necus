...
// The REAL compensation lives in the CCSGOInput::CreateMove hook (Hooks.cpp),
// because that is where the command actually sent to the server is built — the
// same proven path silent aim uses. Writing dwViewAngles at FSN stage 5 (the old
// approach) ran AFTER the command was already built, so it never affected the
// shot. See hkCSGOInputCreateMove for the two compensation modes:
//   • Silent: subtract the full punch from the subtick frame-history angles
//             (server-side only, camera never moves).
//   • Legit : subtract the punch delta from dwViewAngles before the command is
//             built (camera physically pulls down like manual recoil control).
//
// Here we only keep the FSN-stage-0 bookkeeping used by the debug overlay.
namespace RCS {
    // Called from FSN stage 0: export shot counter / punch for the debug overlay.
...
static void ApplyActiveProfile(C_CSPlayerPawn* local)
{
    ...
    Globals::rcs_silent        = p.rcs_silent;
...
// Deprecated: RCS compensation moved into the CreateMove hook (the only place
// that affects the command sent to the server). Kept as a no-op so the FSN5
// call site and header signature stay stable.

...
// Read object type id from object_info_t (+176 offset).
// Returns -1 on bad pointer.
static int GetObjectId(void* objInfo) {
    if (!objInfo) return -1;
    __try { return *(int*)((uintptr_t)objInfo + 176); }
                                     ...
                                     void* sceneView, void* sceneLayer, void* u1, void* u2)
{
    ...
    if (!meshData || !meshData->pObjectInfo ||
        !(Globals::enemy_chams_enabled || Globals::team_chams_enabled)) {
        oDrawArray(a1, a2, meshData, nCount, sceneView, sceneLayer, u1, u2);
        return;
    }

    // Identify entity type via object_info_t::e_id at +176
    const int id = GetObjectId(meshData->pObjectInfo);
    const bool isPlayer = (id == kObjectId_PlayerT || id == kObjectId_PlayerCT);
    if (!isPlayer) {
        oDrawArray(a1, a2, meshData, nCount, sceneView, sceneLayer, u1, u2);
        return;
    }

    // Determine team: T=104 enemy when local is CT and vice-versa. We treat all
    // players as enemies for now when enemy chams are on; team chams get their own path
    // when we have local team info. Simplest reliable approach: if enemy_chams_enabled,
    // apply to all players; if only team_chams_enabled, also apply to all.

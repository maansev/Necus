...
struct BindState {
    ...
    bool* target = nullptr; // the bool this bind controls
    bool     togState = false;   // current state for Toggle mode
    bool     prevKey = false;   // edge detection for Toggle
    bool     active = false;   // current runtime active state (read-only, set by UpdateBinds)
};
...
void Menu::UpdateBinds() {
    for (auto& [lbl, b] : g_binds) {
        ...
        if (b.vk == 0) { b.active = false; continue; }
        bool keyDown = (GetAsyncKeyState(b.vk) & 0x8000) != 0;
        switch (b.mode) {
        case BindMode::Hold:
            b.active = keyDown;
            *b.target = keyDown;
...
static void RenderBindPopup() {
    ...
    if (g_bindCapturing && (ImGui::GetTime() - g_bindCaptureStart) > 0.12) {
        for (int vk = 1; vk < 256; ++vk) {
            if (vk == VK_INSERT || vk == VK_RBUTTON) continue;
            if (GetAsyncKeyState(vk) & 0x8000) {
                g_binds[g_bindPopupLabel].vk = (vk == VK_ESCAPE) ? 0 : vk;
                g_bindCapturing = false;
                break;
            }
        }
    ...
    // Key button
    bool capturing = g_bindCapturing;
    const char* keyLabel = capturing ? "Press any key..." : VkName(b.vk);
    ...
    if (ImGui::Button(keyLabel, ImVec2(bw, 30))) {
        g_bindCapturing = !g_bindCapturing;
        if (g_bindCapturing) g_bindCaptureStart = ImGui::GetTime();
    }
    ImGui::PopStyleColor(4);

    ImGui::Dummy(ImVec2(0, 4));

    // Mode row: "Mode" label left, three buttons fill right
    {
    ...
    float hw = (bw - 4.f) * 0.5f;
    ImGui::PushStyleColor(ImGuiCol_Button, U32(Col::Frame));
    ImGui::PushStyleColor(ImGuiCol_ButtonHovered, U32(ImVec4(0.8f, 0.2f, 0.2f, 0.25f)));
    ImGui::PushStyleColor(ImGuiCol_ButtonActive, U32(ImVec4(0.8f, 0.2f, 0.2f, 0.4f)));
    ImGui::PushStyleColor(ImGuiCol_Text, U32(ImVec4(1.f, 0.45f, 0.45f, 0.9f)));
    bool clearClicked = ImGui::Button("Clear", ImVec2(hw, 26));
    ImGui::PopStyleColor(4);
    if (clearClicked) { b.vk = 0; b.togState = false; b.active = false; g_bindCapturing = false; }
    ...
    bool closeClicked = ImGui::Button("Close", ImVec2(hw, 26));
    ImGui::PopStyleColor(4);

    // Capture window rect BEFORE End() — reliable regardless of focus
    ...
    // One-shot flag: skip all close checks on the frame the popup first appears.
    // Prevents the popup from closing in the same frame it was opened (race with LMB state).
    if (g_bindJustOpened) {
        g_bindJustOpened = false;
        return;
    }

    // Manual rect hover — no dependency on ImGui window focus system
    ...
    if (closeClicked || escJustPressed) {
...
namespace UI {
    ...
    static bool Checkbox(const char* label, bool* v, int* kb = nullptr, bool locked = false) {
        ...
        if (!locked) {
            ...
            if (it != g_binds.end() && it->second.vk != 0) {
...
static void RenderWatermark() {
    ...
    dl->AddRectFilled(ImVec2(x - padX, y - padY), ImVec2(x + totalW + padX, y + th + padY),
        IM_COL32(17, 17, 17, 215), 7.f);
    dl->AddRect(ImVec2(x - padX, y - padY), ImVec2(x + totalW + padX, y + th + padY),
        U32(Col::Border), 7.f, 0, 1.f);
...
static void RenderKeybindList() {
    ...
    struct Entry { std::string label; int vk; bool active; };
    std::vector<Entry> entries;
    for (auto& [lbl, bs] : g_binds) {
        if (bs.vk != 0) entries.push_back({ lbl, bs.vk, bs.active });
    }
    ...
    float maxW = 0.f;
    for (auto& e : entries) {
        char tmp[128]; snprintf(tmp, sizeof(tmp), "%s  [%s]", e.label.c_str(), VkName(e.vk));
        float w = ImGui::CalcTextSize(tmp).x;
        if (w > maxW) maxW = w;
    }
    maxW += 22.f;
    float x = io.DisplaySize.x - maxW - 14.f;
    float y = io.DisplaySize.y - lh * entries.size() - 22.f;
    dl->AddRectFilled(ImVec2(x - 10, y - 8), ImVec2(x + maxW, y + lh * entries.size() + 10),
        IM_COL32(17, 17, 17, 205), 7.f);
    dl->AddRect(ImVec2(x - 10, y - 8), ImVec2(x + maxW, y + lh * entries.size() + 10),
        U32(Col::Border), 7.f, 0, 1.f);
    for (int i = 0; i < (int)entries.size(); ++i) {
        float ly = y + i * lh;
        char kbuf[16]; snprintf(kbuf, sizeof(kbuf), "[%s]", VkName(entries[i].vk));
...
static void RenderSpectatorList() {
    ...
    __try {
        ...
        maxW += 24.f;
        // position below watermark
        float wmH = ImGui::GetTextLineHeight() + 20.f;
        float x = 12.f, y = 12.f + wmH + 6.f;
        float boxH = tlh + lh * count + 10.f;
        dl->AddRectFilled(ImVec2(x - 10, y - 6), ImVec2(x + maxW, y + boxH),
            IM_COL32(17, 17, 17, 205), 7.f);
        dl->AddRect(ImVec2(x - 10, y - 6), ImVec2(x + maxW, y + boxH),
            U32(Col::Border), 7.f, 0, 1.f);
...
static void RenderMiscTab() {
    ...
    UI::BeginCard("Misc");
    {
        ...
        UI::Checkbox("Show Player Money", &Globals::misc_show_money);
        ...
        {
            static const char* remOpts[] = { "None", "Sniper crosshair", "Scope overlay", "Both" };
            UI::Combo("Removals", &Globals::misc_removals, remOpts, 4);
        }

        UI::Checkbox("Hit marker", &Globals::misc_hit_marker);
        if (Globals::misc_hit_marker) {
            static const char* hms[] = { "None", "CoD", "Bell" };
            UI::Combo("Sound##hm", &Globals::misc_hit_marker_sound, hms, 3);
        }
    }
    UI::EndCard();
...
void Menu::Render()
{
    ...
    // Raw RMB edge detection — used by UI::Checkbox to open bind popup
    {
    ...
    if (g_showAllBinds) {
        ...
        for (auto& [lbl, bs] : g_binds) {
            ...
            ImGui::TextUnformatted(VkName(bs.vk));
            ImGui::PopStyleColor();
            ImGui::SameLine(200);

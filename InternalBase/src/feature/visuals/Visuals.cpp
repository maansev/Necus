...
namespace ThirdPerson {
    ...
    static void Update() {
        ...
        // NOTE: m_bInThirdPerson is now written in the CCSGOInput::CreateMove hook
        // (Hooks.cpp) using the real CCSGOInput object passed as `a1`. That runs on
        // the sim thread right after the engine's reset, so it actually sticks —
        // the old write here used `client + dwCSGOInput` WITHOUT dereferencing the
        // pointer, so it likely never hit the real flag. Branch patch above is kept
        // as a bonus if the pattern ever resolves.
    }
} // namespace ThirdPerson

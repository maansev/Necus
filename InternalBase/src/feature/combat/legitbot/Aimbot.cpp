...
void Aimbot::Run(int stage)
{
    ...
    if (stage == 0) {
        __try {
            ...
            if (s_target && !s_target->IsAlive()) s_target = nullptr;
            ...
            long long msSinceTarget = std::chrono::duration_cast<std::chrono::milliseconds>(
                now - s_targetSince).count();
            long long shotDelayMs = static_cast<long long>(Globals::aimbot_shot_delay * 1000.f);
            bool delayOk = msSinceTarget >= shotDelayMs;

            // ─ Auto fire ──────────────────────────────────────────────────────
            if (Globals::aimbot_auto_fire && delayOk) {

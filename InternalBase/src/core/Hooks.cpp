...
{
    // ── Silent aim ───────────────────────────────────────────────────────────
    // Aimbot::Run(stage=0) stored the target angle in g_silentPitch/Yaw without
    // writing to dwViewAngles — camera never moves. We inject the aim angle into
    // every subtick frame-history entry so the server uses it for the bullet.
    // After calling original we restore the visual angles so rendering is correct.
    // Silent aim is an independent feature now — producer (RunSilentAim) sets
    // g_silentValid only when enabled, keyed, on-target and LMB held.
    bool doSilent = Aimbot::g_silentValid;

    float aimPitch = Aimbot::g_silentPitch;
    float aimYaw   = Aimbot::g_silentYaw;
    Aimbot::g_silentValid = false;   // consume

    const int       count = *reinterpret_cast<int*>(a1 + Offsets::dwFrameHistoryCount);
    const uintptr_t arr   = *reinterpret_cast<uintptr_t*>(a1 + Offsets::dwFrameHistoryArray);
        ...
        __try {
            if (arr && count > 0 && count <= 64) {
                ...
                for (int i = 0; i < safeCount; ++i) {
                    ...
                    float* py = reinterpret_cast<float*>(entry + Offsets::kFrameYawOffset);
                    saved[i] = { *pp, *py };   // save visual angles
                    *pp = aimPitch;            // inject aim angle
                    *py = aimYaw;
                }
            }
    ...
    // Restore visual angles so rendering uses camera direction, not aim direction.
    if (safeCount > 0) {
        __try {
            for (int i = 0; i < safeCount; ++i) {
                uintptr_t entry = arr + static_cast<uintptr_t>(Offsets::kFrameEntrySize) * i;
                *reinterpret_cast<float*>(entry + Offsets::kFramePitchOffset) = saved[i].pitch;
                *reinterpret_cast<float*>(entry + Offsets::kFrameYawOffset)   = saved[i].yaw;
            }
    ...
    if (stage == 0) {
        ...
        __try {
            ...
            if (!pawn || !pawn->IsAlive()) {
                ...
                Movement::RunAutostrafer(); // handles its own null-pawn reset

...
// Чтение карты ключ-значение из файла
...
void Config::Load(const std::string& name)
{
    ...
    ReadBool(map, "misc_hitmarker", Globals::misc_hitmarker);
    ReadInt(map, "misc_hitmarker_sound", Globals::misc_hitmarker_sound);
...
void Config::Save(const std::string& name)
{
    ...
    CreateDirectoryW(dir.c_str(), nullptr); // убедимся, что папка есть
    ...
    WriteBool(f, "misc_hitmarker", Globals::misc_hitmarker);
    WriteInt(f, "misc_hitmarker_sound", Globals::misc_hitmarker_sound);

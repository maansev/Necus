...
static void RenderSkinsTab() {
    ...
    // Общий список названий раскрасок (нужен в нескольких картах)
    std::vector<const char*> pk_names;
    for (const auto& pk : paint_kits) pk_names.push_back(pk.name);
    ...
    UI::BeginCard("Knife");
    {
        ...
        for (const auto& k : knife_items) knife_names.push_back(k.name);
        UI::Combo("Model", &Globals::knife_changer_model, knife_names.data(), (int)knife_names.size());
        UI::Combo("Paint kit", &Globals::knife_changer_paint_kit, pk_names.data(), (int)pk_names.size());
        UI::SliderF("Wear", &Globals::knife_changer_wear, 0.0f, 1.0f, "%.4f");
        UI::SliderI("Seed", &Globals::knife_changer_seed, 0, 1000, "%.0f");
    }
    UI::EndCard();
    ...
    UI::BeginCard("Gloves");
    {
        ...
        std::vector<const char*> glove_names;
        for (const auto& g : glove_items) glove_names.push_back(g.name);
        UI::Combo("Model", &Globals::glove_changer_model, glove_names.data(), (int)glove_names.size());
        UI::Combo("Paint kit", &Globals::glove_changer_paint_kit, pk_names.data(), (int)pk_names.size());
        UI::SliderF("Wear", &Globals::glove_changer_wear, 0.0f, 1.0f, "%.4f");
        UI::SliderI("Seed", &Globals::glove_changer_seed, 0, 1000, "%.0f");
    }
    UI::EndCard();
    ImGui::EndChild();
    ...
    UI::ColTitle("Weapon Skins");
    UI::BeginCard("Apply");
    {
        UI::Checkbox("Enable Skin Changer", &Globals::skin_changer_enabled);
        std::vector<const char*> weapon_names;
        for (const auto& w : weapon_items) weapon_names.push_back(w.name);
        static int selected_weapon = 0;
        UI::Combo("Active weapon", &selected_weapon, weapon_names.data(), (int)weapon_names.size());
        UI::Combo("Paint kit", &Globals::skin_changer_paint_kit, pk_names.data(), (int)pk_names.size());
        UI::SliderF("Wear", &Globals::skin_changer_wear, 0.0f, 1.0f, "%.4f");
        UI::SliderI("Seed", &Globals::skin_changer_seed, 0, 1000, "%.0f");

        ImGui::PushStyleColor(ImGuiCol_FrameBg, Col::Frame);
        ImGui::SetNextItemWidth(-UI::kPadX);
        ImGui::InputTextWithHint("##customname", "Custom name", Globals::skin_changer_custom_name, IM_ARRAYSIZE(Globals::skin_changer_custom_name));
        ImGui::PopStyleColor();
    }
    UI::EndCard();
    ImGui::EndChild();

...
// ─── RCS ─────────────────────────────────────────────────────────────────────
// CS2 subtick: m_aimPunchAngle is only populated AFTER client prediction runs.
// Prediction runs BEFORE FSN stage 5 but AFTER FSN stage 0.
// So we read punch at stage 5 and cache it; the CreateMove hook uses the cache
// for server-side compensation. WriteViewAngles at stage 5 also fixes rendering.
namespace RCS {
    static Vector s_lastPunch  = {};
    static int    s_lastShots  = 0;

    // Called from FSN stage 0: update shot counter export for CreateMove hook.
    ...
    // Called from FSN stage 5: punch is valid here (after prediction).
    // Reads interpolated punch = base + velocity*tickFraction and compensates.
    static void RunAtStage5(C_CSPlayerPawn* local, uintptr_t client) {
        if (!Globals::rcs_enabled) { s_lastPunch = {}; s_lastShots = 0; return; }
        if (Globals::rcs_key != 0 && !(GetAsyncKeyState(Globals::rcs_key) & 0x8000)) {
            s_lastPunch = {}; s_lastShots = 0;
            Aimbot::g_rcsPunchPitch = 0.f; Aimbot::g_rcsPunchYaw = 0.f;
            return;
        }

        int    shots     = 0;
        Vector punchBase = {}, punchVel = {};
        float  tickFrac  = 0.f;
        __try {
            shots     = local->m_iShotsFired();
            punchBase = local->m_aimPunchAngle();
            punchVel  = *reinterpret_cast<Vector*>(reinterpret_cast<uintptr_t>(local) + Offsets::m_aimPunchAngleVel);
            tickFrac  = *reinterpret_cast<float*>(reinterpret_cast<uintptr_t>(local) + Offsets::m_aimPunchTickFraction);
        } __except (EXCEPTION_EXECUTE_HANDLER) { return; }

        tickFrac = std::clamp(tickFrac, 0.f, 1.f);
        Vector punch = { punchBase.x + punchVel.x * tickFrac,
                         punchBase.y + punchVel.y * tickFrac, 0.f };

        bool firing = (shots > 0 && shots > Globals::rcs_delay_shots);

        if (Globals::chams_debug) {
            static int s_n = 0;
            if ((s_n++ % 20) == 0)
                Log::Write("RCS s5: shots=%d punch=(%.3f,%.3f) vel=(%.3f,%.3f) frac=%.3f firing=%d",
                           shots, punch.x, punch.y, punchVel.x, punchVel.y, tickFrac, firing ? 1 : 0);
        }

        Aimbot::g_rcsPunchPitch = punch.x;
        Aimbot::g_rcsPunchYaw   = punch.y;

        Vector recoil = { punch.x * 2.f * Globals::rcs_x_scale,
                          punch.y * 2.f * Globals::rcs_y_scale, 0.f };

        if (!firing) { s_lastPunch = recoil; s_lastShots = shots; return; }

        Vector delta = { recoil.x - s_lastPunch.x, recoil.y - s_lastPunch.y, 0.f };
        s_lastPunch  = recoil;
        s_lastShots  = shots;

        if (std::abs(delta.x) < 0.0001f && std::abs(delta.y) < 0.0001f) return;

        float smooth = std::max(1.f, Globals::rcs_smooth);
        float cx = std::clamp(delta.x / smooth, -Globals::rcs_max_x, Globals::rcs_max_x);
        float cy = std::clamp(delta.y / smooth, -Globals::rcs_max_y, Globals::rcs_max_y);
                                           ...
                                           const Vector& eye, const Vector& view)
{
    ...
    // Head bone in CS2 (bone 7) sits slightly below the actual head hitbox,
    ...
    // aimbot key is pressed.
    {
        ...
        if (local0 && local0->IsAlive()) {
            ...
            __try {
                ...
                RunSilentAim(local0, eye, view);   // independent of the visual aimbot
            } __except (EXCEPTION_EXECUTE_HANDLER) {}
        }
    }

    // Visual aimbot is disabled while the independent Silent Aim is on (like
    // other cheats: silent replaces the moving aim). Silent still runs above.
    if (!Globals::aimbot_enabled || Globals::silent_enabled) {

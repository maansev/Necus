...
static bool g_kbJustClicked = false;// pointer to the VK being rebound
...
static void UpdateKeybindCapture() {
    ...
    // Wait for LMB to be released after the chip was clicked, then capture next key
    if (g_kbJustClicked) {
        if (GetAsyncKeyState(0x01) & 0x8000) return; // LMB still held — keep waiting
        g_kbJustClicked = false;                      // LMB released — fall through to scan
    }
    for (int vk = 1; vk < 256; ++vk) {
        if (GetAsyncKeyState(vk) & 0x8000) {
            *g_kbListening = (vk == 0x1B) ? 0 : vk;
...
static void RenderLegitTab() {
    ...
    {
        UI::Checkbox("Enabled", &Globals::aimbot_enabled, &Globals::aimbot_key);
...
static void RenderSkinsTab() {
    ...
    UI::BeginCard("Apply");
    {
        ...
        if (UI::StyledButton("Apply skin changes", ImVec2(-UI::kPadX, 30), true)) {
            Globals::skin_changer_enabled = true;
            Globals::skin_changer_paint_kit = g_skins.paint + 1;
            Globals::skin_changer_wear = (g_skins.wear < 1) ? 0.001f : g_skins.wear / 100.0f;
            Globals::skin_changer_seed = g_skins.seed;
            Globals::knife_changer_enabled = g_skins.knife_on;
            Globals::knife_changer_model = g_skins.knife_model;
            Globals::knife_changer_paint_kit = g_skins.knife_paint + 1;
            Globals::knife_changer_wear = Globals::skin_changer_wear;
            Globals::knife_changer_seed = g_skins.seed;
            Globals::glove_changer_enabled = g_skins.gloves_on;
            Globals::glove_changer_model = g_skins.glove_model;
            Globals::glove_changer_paint_kit = g_skins.glove_paint + 1;
            Globals::glove_changer_wear = Globals::skin_changer_wear;
            Globals::glove_changer_seed = g_skins.seed;
        }
    }
    UI::EndCard();
    ImGui::EndChild();
...
static void RenderMiscTab() {
    ...
    UI::BeginCard("Movement");
    {
        ...
        if (Globals::misc_bhop) {
            static const char* s_bhop_modes[] = { "Rage", "Legit" };
            UI::Combo("Mode##bhop", &Globals::misc_bhop_mode, s_bhop_modes, 2);
            if (Globals::misc_bhop_mode == 1)
                UI::SliderI("Chance##bhop", &Globals::misc_bhop_chance, 50, 100, "%.0f%%");
        }

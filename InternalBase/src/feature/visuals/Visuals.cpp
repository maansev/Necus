...
namespace ThirdPerson {
    ...
    static void Update() {
        ...
        // Пишем ТОЛЬКО когда thirdperson включён. Запись 0 каждый кадр мешала бы
        // штатной камере (смерть/наблюдение). Вызывается из FSN stage 5 (sim-поток),
        // после движкового сброса камеры — поэтому значение держится весь кадр.
        if (!Globals::misc_thirdperson) return;

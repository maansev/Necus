...
namespace Offsets
{
    ...
    constexpr uintptr_t dwGameRules              = 0x2341158;  // client.dll → CCSGameRules* (дамп 2026-06-15)
    constexpr uintptr_t m_bTeamIntroPeriod       = 0xF04;      // bool — заставка "Первая половина"
    constexpr uintptr_t dwCSGOInput              = 0x2356240;  // client.dll → CCSGOInput (дамп 2026-06-15)
    // CCSGOInput::m_bInThirdPerson — НЕ из схемы. 0x229 по форумным источникам.
    constexpr uintptr_t m_bInThirdPerson         = 0x229;
    // Патч ветки сброса камеры (client.dll) — байт jnz по +7 меняем 0x75->0xEB,
    // чтобы движок не сбрасывал thirdperson каждый тик (иначе дёргает онлайн).
    ...
    // CS2 update 2026-04-21 moved aim punch into CCSPlayer_AimPunchServices.
    // m_aimPunchAngle above is now a dead/zeroed field. The live, applied punch
    // is the LAST element of m_aimPunchCache (CUtlVector<QAngle>) on the pawn.
    // CUtlVector layout: int m_Size @ +0x0, QAngle* m_pData @ +0x8.
    constexpr uintptr_t m_pAimPunchServices        = 0x1490;   // ptr (dump 2026-06-15)
    constexpr uintptr_t m_aimPunchSvcBaseAngle     = 0x50;    // CCSPlayer_AimPunchServices::m_predictableBaseAngle QAngle
    constexpr uintptr_t m_aimPunchSvcBaseAngleVel  = 0x5C;    // m_predictableBaseAngleVel QAngle
    constexpr uintptr_t m_aimPunchCache            = 0x14F0;   // CUtlVector<QAngle> (dead since 2026-04-21)
    // Subtick RCS: actual_punch = base + vel * tickFraction
    constexpr uintptr_t m_aimPunchAngleVel    = 0x16F0;
    constexpr uintptr_t m_aimPunchTickFraction = 0x1700;
    ...
    constexpr uintptr_t m_flDuckSpeed       = 0x410;
    constexpr uintptr_t m_vecVelocity       = 0x430;
    constexpr uintptr_t m_duckUntilOnGround = 0x438;

    // CPlayer_MovementServices pointer on pawn (confirmed cs2-dumper 2026-06-10)
    constexpr uintptr_t m_pMovementServices       = 0x1220;
    // Relative to m_pMovementServices pointer:
    constexpr uintptr_t m_flCmdForwardMove        = 0x1A0;  // raw cmd W/S
    constexpr uintptr_t m_flCmdLeftMove           = 0x1A4;  // raw cmd A/D
    constexpr uintptr_t m_arrForceSubtickMoveWhen = 0x1B0;  // float32[4] subtick timing
    constexpr uintptr_t m_flForwardMove           = 0x1C0;  // processed W/S
    constexpr uintptr_t m_flLeftMove              = 0x1C4;  // processed A/D
    ...
    constexpr uintptr_t m_pInGameMoneyServices = 0x808;   // дамп 2026-06-15
    ...
    // CCSPlayerBase_CameraServices::m_flObserverChaseDistance — управляет дистанцией
    // камеры в thirdperson (движок CS2 использует observer-chase камеру). Дамп 2026-06-15.
    ...
    // C_BasePlayerWeapon::m_iClip1 — current rounds in the magazine. Used by the
    // rage auto-fire ammo gate. If this is stale on a future build the gate
    // degrades gracefully (it only blocks firing on a *plausible* 0 read).
    constexpr uintptr_t m_iClip1               = 0x16D8;  // подтверждён дампом 2026-06-15 (был 0x1638 — устаревший)
    ...
    // ==================== Triggerbot / Aimbot (порт из aimbot.h + triggerbot.h) ====================
    // m_nCrosshairId — индекс entity под прицелом (C_CSPlayerPawn + 0x3EAC)
    constexpr uintptr_t m_nCrosshairId = 0x3EAC;

    // EntitySpottedState_t (inline в C_CSPlayerPawn).
    // +0x0 = m_bSpotted (bool) — true если entity замечена хотя бы одним игроком.
    // Подтверждён дампом 2026-06-15: m_entitySpottedState = 0x1CC8 (был 0x1C38 — устаревший).
    constexpr uintptr_t m_entitySpottedState = 0x1CC8;

    // ==================== Сигнатуры паттернов ====================
    // CCSGOInput::CreateMove (client.dll) — используется для frame-history silent aim
    constexpr const char* sig_CreateMove_client = "48 8B C4 4C 89 40 18 48 89 48 08 55 53 41 54 41 55";

    // ==================== CCSGOInput frame history (для silent aim через CreateMove) ====================
    constexpr uintptr_t dwFrameHistoryCount = 0xBC8; // int   — кол-во записей
    constexpr uintptr_t dwFrameHistoryArray = 0xBD0; // ptr   — массив записей
    // Каждая запись: 96 байт (0x60). pitch @ +0x10, yaw @ +0x14
    constexpr int kFrameEntrySize    = 96;
    constexpr int kFramePitchOffset  = 0x10;
    constexpr int kFrameYawOffset    = 0x14;
}

...
void Hooks::Setup()
{
    ...
    if (CreateInterface)
    {
        ...
        if (client)
        {
            ...
            // --- FrameStageNotify hook (скинчейнджер) ---
            {
                void** clientModeVMT = *(void***)clientMode;
                void* frameStageAddr = clientModeVMT[37];   // проверить индекс (обычно 37)
                if (frameStageAddr) {
                    MH_CreateHook(frameStageAddr, &hkFrameStageNotify, (void**)&oFrameStageNotify);
                }
            }
        }
    }

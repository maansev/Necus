...
double __fastcall hkCSGOInputCreateMove(__int64 a1, unsigned int a2, __int64 a3)
{
    ...
    bool needsFrameModify = doSilent || doRcsSilent;
    if (needsFrameModify) {
        __try {
            if (arr && count > 0 && count <= 64) {
                ...
                for (int i = 0; i < safeCount; ++i) {
                    ...
                    float np = doSilent ? aimPitch : *pp;
                    float ny = doSilent ? aimYaw   : *py;
    ...
    // ── Third person ─────────────────────────────────────────────────────────
    __try {
        *reinterpret_cast<uint8_t*>(a1 + Offsets::m_bInThirdPerson) =
            Globals::misc_thirdperson ? 1 : 0;
    } __except (EXCEPTION_EXECUTE_HANDLER) {}
...
HRESULT __stdcall Hooks::hkPresent(IDXGISwapChain* swapChain, UINT sync, UINT flags)
{
    ...
    // Autostrafe: write target yaw at render frequency so it overrides SDL mouse input.
    // CreateMove runs at ~64Hz but SDL mouse runs at 200+Hz and overwrites va[1] between ticks.
    // Present runs at render rate and wins the race reliably.
    if (Movement::g_strafeActive && client) {
        __try {
            auto* va = reinterpret_cast<float*>(client + DW_VIEW_ANGLES);
            va[1] = Movement::g_strafeTargetYaw;
        } __except (EXCEPTION_EXECUTE_HANDLER) {}
    }

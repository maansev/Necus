    ...
    const char* custom_name, uint16_t def_index)
{
    ...
    // Force CS2 fallback / override path (engine reads m_nFallback* when IDHigh == -1)
...
void c_skin_changer::process_knife(C_EconEntity* weapon, C_EconItemView* item, bool force)
{
    ...
    // Swap model — needed for knife model to visually change
...
void c_skin_changer::run_inner()
{
    ...
    // CS2: CPlayer_WeaponServices::m_hMyWeapons is at +0x70
    auto* myWeapons = reinterpret_cast<CUtlVectorSimple*>(weapSvc + 0x70);
    ...
    for (int i = 0; i < count; ++i) {
        ...
        // Knives: default (42/59) + all custom knife def indices (500-527)

...
{
    return GetConfigDir() + std::wstring(name.begin(), name.end()) + L".cfg";
}
...
void Config::Load(const std::string& name)
{
    ...
    // Rage
    ReadBool(map,  "rage_enabled",      Globals::rage_enabled);
    ReadInt(map,   "rage_key",          Globals::rage_key);
    ReadFloat(map, "rage_fov",          Globals::rage_fov);
    ReadInt(map,   "rage_refine_shot",  Globals::rage_refine_shot);
    ReadBool(map,  "rage_auto_fire",    Globals::aimbot_auto_fire);
    ReadBool(map,  "rage_auto_wall",    Globals::aimbot_auto_wall);
    ReadBool(map,  "rage_silent",       Globals::silent_enabled);
    ReadInt(map,   "rage_silent_key",   Globals::silent_key);
    // Per-weapon rage profiles
    for (int i = 0; i < 35; ++i) {
        std::string pfx = "rp" + std::to_string(i) + "_";
        ReadInt(map,  (pfx + "target").c_str(),    Globals::g_rageProfiles[i].target);
        ReadInt(map,  (pfx + "hitchance").c_str(), Globals::g_rageProfiles[i].hitchance);
        ReadInt(map,  (pfx + "mindmg").c_str(),    Globals::g_rageProfiles[i].mindmg);
        for (int j = 0; j < 6; ++j)
            ReadBool(map, (pfx + "hb" + std::to_string(j)).c_str(), Globals::g_rageProfiles[i].hitboxes[j]);
        for (int j = 0; j < 3; ++j)
            ReadBool(map, (pfx + "mp" + std::to_string(j)).c_str(), Globals::g_rageProfiles[i].multipoint[j]);
    }
    // Anti-aim
    ReadBool(map,  "aa_enabled",    Globals::aa_enabled);
    ReadInt(map,   "aa_key",        Globals::aa_key);
    ReadInt(map,   "aa_pitch",      Globals::aa_pitch);
    ReadInt(map,   "aa_yaw",        Globals::aa_yaw);
    ReadFloat(map, "aa_spin_speed", Globals::aa_spin_speed);
    ReadFloat(map, "aa_yaw_base",   Globals::aa_yaw_base);
    ReadFloat(map, "aa_jitter",     Globals::aa_jitter);
    ...
    ReadInt(map,  "misc_hit_marker_style",     Globals::misc_hit_marker_style);
...
void Config::Save(const std::string& name)
{
    ...
    // Rage
    WriteBool(f,  "rage_enabled",      Globals::rage_enabled);
    WriteInt(f,   "rage_key",          Globals::rage_key);
    WriteFloat(f, "rage_fov",          Globals::rage_fov);
    WriteInt(f,   "rage_refine_shot",  Globals::rage_refine_shot);
    WriteBool(f,  "rage_auto_fire",    Globals::aimbot_auto_fire);
    WriteBool(f,  "rage_auto_wall",  Globals::aimbot_auto_wall);
    WriteBool(f,  "rage_silent",     Globals::silent_enabled);
    WriteInt(f,   "rage_silent_key", Globals::silent_key);
    // Per-weapon rage profiles
    for (int i = 0; i < 35; ++i) {
        std::string pfx = "rp" + std::to_string(i) + "_";
        WriteInt(f,  (pfx + "target").c_str(),    Globals::g_rageProfiles[i].target);
        WriteInt(f,  (pfx + "hitchance").c_str(), Globals::g_rageProfiles[i].hitchance);
        WriteInt(f,  (pfx + "mindmg").c_str(),    Globals::g_rageProfiles[i].mindmg);
        for (int j = 0; j < 6; ++j)
            WriteBool(f, (pfx + "hb" + std::to_string(j)).c_str(), Globals::g_rageProfiles[i].hitboxes[j]);
        for (int j = 0; j < 3; ++j)
            WriteBool(f, (pfx + "mp" + std::to_string(j)).c_str(), Globals::g_rageProfiles[i].multipoint[j]);
    }
    // Anti-aim
    WriteBool(f,  "aa_enabled",    Globals::aa_enabled);
    WriteInt(f,   "aa_key",        Globals::aa_key);
    WriteInt(f,   "aa_pitch",      Globals::aa_pitch);
    WriteInt(f,   "aa_yaw",        Globals::aa_yaw);
    WriteFloat(f, "aa_spin_speed", Globals::aa_spin_speed);
    WriteFloat(f, "aa_yaw_base",   Globals::aa_yaw_base);
    WriteFloat(f, "aa_jitter",     Globals::aa_jitter);
    ...
    WriteInt(f,   "misc_hit_marker_style",     Globals::misc_hit_marker_style);

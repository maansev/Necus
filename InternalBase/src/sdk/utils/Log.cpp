...
static LONG CALLBACK VehHandler(PEXCEPTION_POINTERS ep) {
    ...
    // ── Throttle ──────────────────────────────────────────────────────────────
    // Most of these are FIRST-CHANCE access violations that our own __try/__except
    // blocks intentionally swallow (volatile game reads during respawn / map load).
    // Logging every one — each call walks all loaded modules in DescribeAddress and
    // hits the disk — turns a transient fault storm into an FPS collapse. Log at
    // most one exception per window; collapse the rest into a suppressed count.
    ...
    if (nowMs - last < kWindowMs) {
        s_suppressed.fetch_add(1, std::memory_order_relaxed);
        return EXCEPTION_CONTINUE_SEARCH;   // cheap path — no module walk, no I/O
    }
    ...
    uint32_t skipped = s_suppressed.exchange(0, std::memory_order_relaxed);
    ...
    if (code == EXCEPTION_ACCESS_VIOLATION && ep->ExceptionRecord->NumberParameters >= 2) {
        ...
        Log::Write("!!! EXCEPTION code=0x%08X at %s — AV %s of 0x%p | checkpoint=%s | suppressed=%u",
                   code, where, op, (void*)ep->ExceptionRecord->ExceptionInformation[1],
                   (const char*)tl_checkpoint, skipped);
    } else {
        Log::Write("!!! EXCEPTION code=0x%08X at %s | checkpoint=%s | suppressed=%u",
                   code, where, (const char*)tl_checkpoint, skipped);
    }
    DumpCrumbs(); // full step-by-step trail leading to the crash (once per session)
...
void Log::Init() {
    ...
    // Try multiple locations in order of likelihood to be writable.
    const char* kPaths[] = {
        "C:\\necus_log.txt",        // root C: (admin / some launchers)
        nullptr,                    // placeholder — filled with Desktop path below
        nullptr,                    // placeholder — filled with Documents path below
        nullptr,                    // placeholder — filled with %TEMP% path below
    };
    ...
    if (SUCCEEDED(SHGetFolderPathA(nullptr, CSIDL_DESKTOP, nullptr, 0, desktopPath)))
        lstrcatA(desktopPath, "\\necus_log.txt");
    if (SUCCEEDED(SHGetFolderPathA(nullptr, CSIDL_PERSONAL, nullptr, 0, docsPath)))
        lstrcatA(docsPath, "\\necus_log.txt");
    if (GetTempPathA(MAX_PATH, tempPath))
        lstrcatA(tempPath, "necus_log.txt");

    kPaths[1] = desktopPath[0] ? desktopPath : nullptr;
    kPaths[2] = docsPath[0]    ? docsPath    : nullptr;
    kPaths[3] = tempPath[0]    ? tempPath    : nullptr;

    for (int i = 0; i < 4 && g_logFile == INVALID_HANDLE_VALUE; ++i) {
        if (!kPaths[i] || !kPaths[i][0]) continue;
        g_logFile = CreateFileA(kPaths[i], GENERIC_WRITE, FILE_SHARE_READ,

...
void __fastcall hkFrameStageNotify(void* ecx, int stage)
{
    ...
    if (stage == 0) {
        ...
        __try {
            ...
            if (!pawn || !pawn->IsAlive()) {
                *(uint32_t*)(client + BUTTON_JUMP) = BTN_RELEASED;
            ...
            if (Globals::misc_infinite_duck) {
                *(float*)((uintptr_t)pawn + Offsets::m_flDuckAmount) = 1.0f;
                *(bool*)((uintptr_t)pawn + Offsets::m_bDucked) = true;
                *(uint32_t*)(client + BUTTON_DUCK) = BTN_PRESSED;
            }
            // When off: don't touch BUTTON_DUCK — let the game/player handle it normally
    ...
    // ── Stage 6: camera-lock — сохраняем углы ────────────────────────────────
    if (stage == 6) {
        if (Menu::IsOpen) {
            uintptr_t client = Memory::GetModuleBase("client.dll");
    ...
    // ── Stage 7: скины, silent aimbot restore, camera-lock restore ────────────
    if (stage == 7) {
        c_skin_changer::get().run(stage);
        c_glove_changer::get().run(stage);

        // Silent aimbot: восстановить визуальный угол
        Aimbot::Run(7);
...
HRESULT __stdcall Hooks::hkPresent(IDXGISwapChain* swapChain, UINT sync, UINT flags)
{
    ...
    if (GetAsyncKeyState(VK_INSERT) & 1) {
        ...
        if (Menu::IsOpen) {
            ::ShowCursor(TRUE); ::ClipCursor(nullptr);
...
void Hooks::Setup()
{
    ...
    { // Chams DrawArray
        ...
        if (addr) MH_CreateHook(reinterpret_cast<void*>(addr), &chams::DrawArrayHook, (void**)&oDrawArray);
    }
    { // FrameStageNotify

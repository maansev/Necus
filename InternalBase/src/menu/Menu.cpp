...
// True if a feature label has at least one key bound. Used so that sub-options
// of a bound feature stay visible even while the feature is currently inactive.
static bool IsBound(const char* label) {
    auto it = g_binds.find(label);
    return it != g_binds.end() && !it->second.vks.empty();
}
...
void Menu::UpdateBinds() {
    ...
    // Sync NecusState::RageConfig anti-aim fields → Globals so AntiAim::Compute
    // reads live menu values. RageConfig.pitch2/yaw2 indices match aa_pitch/aa_yaw.
    {
        using namespace NecusState;
        Globals::aa_enabled = g_rage.aa_on;
        Globals::aa_key     = g_rage.kb_aa;
        Globals::aa_pitch   = g_rage.pitch2;
        Globals::aa_yaw     = g_rage.yaw2;
    }
}
...
namespace UI {
    ...
    static void Color(const char* label, float* rgba) {
        ...
        ImGui::ColorEdit4("##col", rgba, ImGuiColorEditFlags_NoInputs | ImGuiColorEditFlags_NoLabel | ImGuiColorEditFlags_AlphaBar | ImGuiColorEditFlags_AlphaPreview);
        ImGui::PopID();
    }
...
static void RenderRageTab() {
    ...
    UI::BeginCard("Main");
    {
        UI::Checkbox("Enabled",          &Globals::rage_enabled,      &Globals::rage_key);
        UI::Checkbox("Silent Aim",        &Globals::silent_enabled,    &Globals::silent_key);
        UI::Checkbox("Automatic Fire",    &Globals::aimbot_auto_fire);
        UI::Checkbox("Aim Through Walls", &Globals::aimbot_auto_wall);
        UI::SliderF("Field of View", &Globals::rage_fov, 0.f, 180.f, "%.1f\xC2\xB0");
    }
    UI::EndCard();

    UI::ColTitle("Selection");

    UI::BeginCard("Selection", kWeapons[g_rageWeapon]);
    {
        auto& rp = Globals::g_rageProfiles[g_rageWeapon];
        static const char* tgt[] = {
            "Highest Damage", "Highest Hit Chance", "Closest", "Lowest HP", "Random"
        };
        UI::Combo("Target", &rp.target, tgt, 5);
        static const char* hb[] = { "Head", "Chest", "Stomach", "Arms", "Legs", "Feet" };
        UI::MultiCombo("Hitboxes", hb, rp.hitboxes, 6);
        static const char* mp[] = { "Head", "Chest", "Stomach" };
        UI::MultiCombo("Multipoint", mp, rp.multipoint, 3);

        // ── Hit Chance row with "..." popup ──────────────────────────────────
        {
            float avail = ImGui::GetContentRegionAvail().x;
            float btnW  = 28.f;
            float sliderW = avail - btnW - ImGui::GetStyle().ItemSpacing.x;
            ImGui::SetNextItemWidth(sliderW);
            ImGui::SliderInt("##hc", &rp.hitchance, 0, 100,
                             rp.hitchance <= 0 ? "Auto" : "%d%%");
            ImGui::SameLine();
            const char* hcPopupId = "##hcpop";
            if (ImGui::Button("...", ImVec2(btnW, 0))) ImGui::OpenPopup(hcPopupId);
            ImGui::SameLine(0, 4.f);
            ImGui::TextUnformatted("Hit Chance");

            if (ImGui::BeginPopup(hcPopupId)) {
                ImGui::TextUnformatted("Hit Chance");
                ImGui::Separator();
                ImGui::Checkbox("Force Shot", &rp.force_shot);
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Always shoot regardless of hit-chance gate");
                float hcF = (float)rp.hitchance;
                ImGui::SetNextItemWidth(160.f);
                if (ImGui::SliderFloat("Min. Hit Chance", &hcF, 0.f, 100.f, "%.0f%%"))
                    rp.hitchance = (int)hcF;
                ImGui::SetNextItemWidth(160.f);
                ImGui::SliderFloat("Extrapolation Safety", &rp.extrapolation,
                                   0.f, 64.f, "%.1f u");
                if (ImGui::IsItemHovered())
                    ImGui::SetTooltip("Extra latency buffer (units) added to hit-chance movement gate");
                ImGui::EndPopup();
            }
        }

        // Max 105 (enough to lethal a full-HP+armor target); 0 shows "Auto",
        // where the bot auto-picks the damage needed per target (see RageTarget).
        UI::SliderI("Min Damage",  &rp.mindmg,    0, 105,
                    rp.mindmg <= 0 ? "Auto" : "%.0f");
        UI::Checkbox("Quick Stop",  &Globals::misc_quickstop);
        if (Globals::misc_quickstop) {
            static const char* qsm[] = { "In Air", "On Land", "Between Shots" };
            ImGui::Indent(14.f);
            UI::Combo("##qsmode", &Globals::misc_quickstop_mode, qsm, 3);
            ImGui::Unindent(14.f);
        }
        UI::Checkbox("Quick Scope", &Globals::aimbot_auto_scope);
    }
    UI::EndCard();
    ImGui::EndChild();
    ...
    UI::BeginCard("Other");
    {
        ...
        static const char* ref[] = { "Off", "Latency", "Performance" };
        UI::Combo("Refine Shot", &Globals::rage_refine_shot, ref, 3);
...
static void RenderLegitTab() {
    ...
    auto& prof = Globals::g_legitProfiles[g_legitWeapon];

    // ── Left column: GLOBALS + TRIGGERBOT ────────────────────────────────────
    ImGui::BeginChild("legitA", ImVec2(colW, 0), false, ImGuiWindowFlags_NoScrollbar);
    UI::ColTitle("Globals");
    UI::BeginCard("Globals");
    {
        static const char* dw[] = { "Smoke", "Flash", "Reloading" };
        UI::MultiCombo("Disable when", dw, Globals::aimbot_disable_on, 3);
        ...
        UI::Checkbox("Silent aim",   &Globals::silent_enabled);
    ...
    {
        UI::Checkbox("Enable Triggerbot", &Globals::triggerbot_enabled, &Globals::triggerbot_key);
        if (Globals::triggerbot_enabled || IsBound("Enable Triggerbot")) {
            UI::SliderI("Delay (ms)", &Globals::triggerbot_delay, 0, 500, "%.0f");
            static const char* tbhb[] = { "Head", "Neck", "Chest", "Stomach", "Arms", "Legs" };
            UI::MultiCombo("Hitboxes", tbhb, Globals::triggerbot_hitboxes, 6);
        }
    ...
    // ── Right column: AIMBOT (per-weapon) + RCS (per-weapon) ─────────────────
    ImGui::BeginChild("legitB", ImVec2(0, 0), false, ImGuiWindowFlags_NoScrollbar);
    UI::ColTitle("Aimbot");
    UI::BeginCard("Aimbot", badge);
    {
        UI::Checkbox("Enabled", &prof.aimbot_enabled);
        if (prof.aimbot_enabled || IsBound("Enabled")) {
            static const char* hb[] = { "Head", "Neck", "Chest", "Stomach", "Arms", "Legs" };
            UI::MultiCombo("Hitboxes", hb, prof.hitboxes, 6);
            UI::SliderF("FOV",        &prof.fov,         0.f,  45.f, "%.2f");
            UI::SliderF("Smooth",     &prof.smooth,      1.f,  20.f, "%.2f");
            UI::SliderF("Shot delay", &prof.shot_delay,  0.f,   0.5f, "%.2f");
            UI::SliderF("Kill delay", &prof.kill_delay,  0.f,   1.0f, "%.2f");
            UI::SliderF("Min damage", &prof.min_damage,  0.f, 100.f,
                        prof.min_damage <= 0.f ? "auto" : "%.0f hp");
            UI::SliderF("Hit chance", &prof.hit_chance,  0.f, 100.f,
                        prof.hit_chance <= 0.f ? "auto" : "%.0f%%");
            UI::Checkbox("Lock target", &prof.lock_target);
            UI::Checkbox("Lock mouse",  &prof.lock_mouse);
        }
    }
    UI::EndCard();

    UI::ColTitle("RCS");
    UI::BeginCard("RCS", badge);
    {
        UI::Checkbox("Enable RCS", &prof.rcs_enabled, &prof.rcs_key);
        if (prof.rcs_enabled || IsBound("Enable RCS")) {
            UI::SliderF("Pitch scale",     &prof.rcs_x_scale,      0.f,  2.f,   "%.2f");
            UI::SliderF("Yaw scale",       &prof.rcs_y_scale,      0.f,  2.f,   "%.2f");
            UI::SliderF("Max pitch/frame", &prof.rcs_max_x,         0.f, 10.f,  "%.1f");
            UI::SliderF("Max yaw/frame",   &prof.rcs_max_y,         0.f, 10.f,  "%.1f");
            UI::SliderI("Shot delay",      &prof.rcs_delay_shots,   0,    5,    "%.0f shots");
        }
    }
    UI::EndCard();
    ImGui::EndChild();
...
static void RenderVisualsTab() {
    ...
    if (g_visualsSub == 0 || g_visualsSub == 1) {
        ...
        if (*(e ? &Globals::enemy_chams_enabled : &Globals::team_chams_enabled)) {
            ...
            UI::Checkbox("Debug (log to C:\\necus_log.txt)", &Globals::chams_debug);
        }
        UI::EndCard();
...
// A canonical user-mode heap pointer is non-null, above the first 64 KiB and
// below the 0x7FFF'FFFFFFFF user-space ceiling. OvSafeRead returns 0 on fault
// but a *garbage non-zero* value (e.g. 0xFFFFFFFF........) still passes a plain
// `if (!ptr)` check and then faults on the next deref — flooding the VEH log.
// Use this to reject implausible pointers before dereferencing them.
static inline bool OvPtrOk(uintptr_t p) {
...
static uintptr_t OvGetEnt(uintptr_t listPtr, int i) {
    __try {
        ...
        if (!OvPtrOk(chunk)) return 0;
        uintptr_t ent = OvSafeRead(chunk + 112 * (i & 0x1FF));
        return OvPtrOk(ent) ? ent : 0;
    } __except (EXCEPTION_EXECUTE_HANDLER) { return 0; }
}
    ...
    __try {
        ...
        for (int i = 1; i < 64 && count < 20; ++i) {
            __try {
                ...
                if (!OvPtrOk(ctrl) || ctrl == localCtrl) continue;
                ...
                if (!OvPtrOk(pawnChunk)) continue;
                uintptr_t pawn = OvSafeRead(pawnChunk + 112 * (ph & 0x1FF));
                if (!OvPtrOk(pawn)) continue;
                uintptr_t obsSvc = OvSafeRead(pawn + Offsets::m_pObserverServices);
                if (!OvPtrOk(obsSvc)) continue;
...
static void RenderMiscTab() {
    ...
    UI::BeginCard("Movement");
    {
        ...
        UI::Checkbox("Auto Pistol",   &Globals::misc_auto_pistol);
    }
    UI::EndCard();
    ...
    UI::BeginCard("Misc");
    {
        ...
        UI::Checkbox("Bullet Tracer", &Globals::misc_hit_marker);
        if (Globals::misc_hit_marker) {
            static const char* styleOpts[] = { "Classic", "Animated", "Bloom" };
            UI::Combo("Style##hm", &Globals::misc_hit_marker_style, styleOpts, 3);

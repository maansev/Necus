...
#include "../feature/misc/skin_changer.h"  // пока что создадим этот файл
...
LRESULT __stdcall Hooks::hkWndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    if (Menu::IsOpen)
    {
        ...
        if (ImGui::GetIO().WantCaptureKeyboard &&
            (msg == WM_KEYDOWN || msg == WM_KEYUP || msg == WM_SYSKEYDOWN || msg == WM_SYSKEYUP || msg == WM_CHAR)) return true;
...
HRESULT __stdcall Hooks::hkPresent(IDXGISwapChain* swapChain, UINT sync, UINT flags)
{
    if (!g_Init)
    {
        ...
        NecusSetLogo(LoadNecusLogo(g_Device));   // logo from logo_rgba.h (raw RGBA, no WIC/COM)   // logo from logo.h
    ...
    // === ДИАГНОСТИКА: каждая точка всплывает ОДИН раз. Последнее окно перед
    //     крашем = виновная подсистема. Удали этот блок и DIAG-метки когда найдём. ===
#define DIAG(msg) { static bool _s=false; if(!_s){_s=true; MessageBoxA(nullptr, msg, "Necus CP", MB_OK|MB_TOPMOST);} }
    DIAG("CP1: frame start (init OK)");

    EntityManager::Get().Update();
    DIAG("CP2: EntityManager OK");

    // Скинчейнджер/перчатки — вызываем из Present (надёжный хук), а не из
    // хрупкого FrameStageNotify. Обе функции SEH-защищены, краш невозможен.
    c_skin_changer::get().run(3);
    c_glove_changer::get().run(3);
    DIAG("CP3: skin/glove changers OK");
    ...
    ImGui::NewFrame();
    DIAG("CP4: ImGui NewFrame OK");

    if (Menu::IsOpen)
        Menu::Render();
    DIAG("CP5: Menu::Render OK (or closed)");

    Visuals::Render();
    DIAG("CP6: Visuals::Render OK");

    ImGui::Render();
    g_Context->OMSetRenderTargets(1, &g_RTV, nullptr);
    ImGui_ImplDX11_RenderDrawData(ImGui::GetDrawData());
    DIAG("CP7: full frame rendered — ALL OK");
    ...
    if (CreateInterface)
    {
        ...
        if (client)
        {
            ...
            if (clientMode)
            {
                ...
                // FrameStageNotify hook УБРАН — был ненадёжен (vmt[37] мог быть неверным
                // индексом → краш). Скинчейнджер теперь вызывается из hkPresent.
            }
        }
    }
...
void __fastcall hkFrameStageNotify(void* thisptr, int stage) {
    ...
    if (stage == 3) { // CS2: FRAME_NET_UPDATE_POSTDATAUPDATE_END

...
// ─── LevelInit hook ──────────────────────────────────────────────────────────
// Fires when a new map loads. Use it to invalidate any per-match caches
// (glow state, skin applied flags, etc.) so server-join transitions stay clean.
...
void* __fastcall hkLevelInit(void* thisptr, const char* szNewMap) {
    ...
    __try { c_inventory_changer::get().invalidate_all(); }
    ...
    if (stage == 0) {
        ...
        __try {
            ...
            if (!pawn || !pawn->IsAlive()) {
                s_bhop_was_ground = false; s_yawInit = false;
            ...
            // Uses the game's button states (client.dll BUTTON_MOVELEFT /
            // BUTTON_MOVERIGHT, BTN_PRESSED = 0x10001 / BTN_RELEASED = 0x100 —
            // same encoding as the bhop jump writes above) instead of CUserCmd
            // bits, because this codebase drives input through direct button
            // writes. Direction follows the per-frame yaw delta of
            // dwViewAngles. Skipped on ladders/noclip.
...
HRESULT __stdcall Hooks::hkPresent(IDXGISwapChain* swapChain, UINT sync, UINT flags)
{
    ...
    __try { EntityManager::Get().Update(); }
    __except (EXCEPTION_EXECUTE_HANDLER) {}
    Log::Checkpoint("Present:UpdateRemovals");
    __try { Visuals::UpdateRemovals(); }
    __except (EXCEPTION_EXECUTE_HANDLER) {}
    // DrawArray-based chams are applied directly in HookDrawArray (per-mesh-draw).
    // The old glow-based chams::Update() is no longer called.
    ...
    __try { Menu::UpdateBinds(); }
    __except (EXCEPTION_EXECUTE_HANDLER) {}
    __try { Misc::RunAutoAccept(g_Window); }
    __except (EXCEPTION_EXECUTE_HANDLER) {}
    ...
    // Override ImGui logical width to match the DXGI source rect so that the
    // projection matrix, menu positions and ESP screen coords all land inside
    // the visible stretched region.
    ...
    __try { if (Menu::IsOpen) Menu::Render(); }
    __except (EXCEPTION_EXECUTE_HANDLER) {}
    Log::Checkpoint("Present:VisualsRender");
    __try { Visuals::Render(); }
    __except (EXCEPTION_EXECUTE_HANDLER) {}
    Log::Checkpoint("Present:Overlays");
    __try { Menu::RenderOverlays(); }
    __except (EXCEPTION_EXECUTE_HANDLER) {}
...
void Hooks::Setup()
{
    ...
    { // LevelInit — reset per-match state on map load
        uintptr_t fn = Memory::PatternScan("client.dll",
            "48 89 5C 24 ? 48 89 74 24 ? 57 48 83 EC 20 48 8B FA 48 8B F1 48 85 D2 0F 84");
        Log::Write("LevelInit pattern: 0x%llX", (unsigned long long)fn);
        if (fn) {
            MH_STATUS s = MH_CreateHook(reinterpret_cast<void*>(fn), &hkLevelInit,
                reinterpret_cast<void**>(&oLevelInit));
            Log::Write("LevelInit hook: %s", MH_StatusToString(s));
        }
    }
    { // DrawArray (scenesystem.dll) — entity-level mesh draw hook for chams
        uintptr_t fn = Memory::PatternScan("scenesystem.dll",
            "48 8B C4 48 89 50 10 53 41 55 41 56 48 81 EC ? ? ? ? 4D 63 F1");
        Log::Write("DrawArray pattern: 0x%llX", (unsigned long long)fn);
        if (fn) {
            if (MH_CreateHook(reinterpret_cast<void*>(fn),
                &Chams::HookDrawArray,
                reinterpret_cast<void**>(&Chams::oDrawArray)) == MH_OK)
                Log::Write("DrawArray hook installed");
            else
                Log::Write("DrawArray MH_CreateHook FAILED");
        }
    }
    { // GetRawInputData

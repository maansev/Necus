...
namespace Globals
{
    ...
    inline bool  enemy_chams_xqz_enabled = false;   // draw through walls (XQZ layer)
    inline float enemy_chams_visible_color[4]   = { 1.0f, 0.5f, 0.0f, 1.0f };
    inline float enemy_chams_invisible_color[4] = { 0.0f, 0.0f, 1.0f, 0.45f };
    inline int   enemy_chams_material = 0;  // 0=Flat 1=Illuminate 2=Glow

    // Diagnostic: log material names the DrawObject hook sees (find player prefix)
    inline bool  chams_debug = false;

    // ==================== TEAM CHAMS ====================
    inline bool  team_chams_enabled = false;
    inline bool  team_chams_xqz_enabled = false;
    inline float team_chams_visible_color[4]   = { 0.0f, 1.0f, 0.0f, 1.0f };
    inline float team_chams_invisible_color[4] = { 0.0f, 0.5f, 1.0f, 0.45f };
    ...
    // ==================== PER-WEAPON LEGIT PROFILE ====================
    // Each slot stores aimbot + RCS settings for one weapon (or Global fallback).
    // Slot 0 = Global (fallback if no per-weapon slot set).
    // Slots 1-34 map to the weapon picker in Menu.cpp (kWeapons[1..34]).
    // Aimbot::Run copies the active slot into the flat Globals:: vars below
    // so that Visuals.cpp / Hooks.cpp / Config.cpp remain unmodified.
    struct LegitWeaponProfile {
        bool  aimbot_enabled    = false;
        bool  hitboxes[6]       = { true, false, false, false, false, false };
        float fov               = 5.0f;
        float smooth            = 5.0f;
        float shot_delay        = 0.05f;
        float kill_delay        = 0.20f;
        ...
        int   rcs_key           = 0;
        float rcs_x_scale       = 1.0f;
        float rcs_y_scale       = 1.0f;
        float rcs_max_x         = 5.0f;
        float rcs_max_y         = 5.0f;
        int   rcs_delay_shots   = 1;
    };
    // 35 slots: [0]=Global, [1..34]=per-weapon (matches kWeapons array in Menu.cpp)
    inline LegitWeaponProfile g_legitProfiles[35];

    // ==================== RAGE ───────────────────────────────────────────────────
    inline bool  rage_enabled    = false;
    inline int   rage_key        = 0x70;   // F1
    inline float rage_fov        = 180.f;  // controls all rage features
    inline int   rage_refine_shot = 0;     // 0=Off 1=Latency 2=Performance

    struct RageWeaponProfile {
        int  target        = 0;           // 0=HighDmg 1=HighestHitChance 2=Closest 3=LowestHP 4=Random
        bool hitboxes[6]   = { true, false, false, false, false, false };
        bool multipoint[3] = { true, false, false };  // Head/Chest/Stomach
        int  hitchance     = 50;          // Min. Hit Chance slider (player speed gate)
        bool force_shot    = false;       // Force Shot: ignore hit-chance gate, fire anyway
        float extrapolation = 0.f;        // Extrapolation Safety (units; adds latency buffer)
        int  mindmg        = 15;
    };
    inline RageWeaponProfile g_rageProfiles[35];   // [0]=Global [1-34]=per-weapon

    // ==================== AIMBOT (runtime / flat — written by Aimbot::Run from active profile) ====================
    ...
    inline bool  aimbot_silent           = false;  // legacy aimbot mode (kept for Config compat)
    inline bool  aimbot_visibility_check = true;

    // ==================== Silent Aim (independent feature) ====================
    // Standalone: own enable + own key. Shares FOV/hitboxes with the active
    // aimbot profile (flat Globals::aimbot_fov / aimbot_hitboxes). Only steers
    // the bullet (subtick frame history) while LMB is held — camera never moves.
    inline bool  silent_enabled          = false;
    inline int   silent_key              = 0;   // 0 = active whenever enabled (still requires LMB)
    ...
    inline int   aimbot_disable_when     = 0;   // legacy (kept for Config compat)
    // Multi-select disable conditions: [0]=Smoke [1]=Flash [2]=Reloading.
    inline bool  aimbot_disable_on[3]    = { false, false, false };
    ...
    // ==================== RCS (runtime / flat — written by Aimbot::Run from active profile) ====================
    inline bool  rcs_enabled        = false;
    inline int   rcs_key            = 0;
    inline bool  rcs_silent         = true;    // true = server-side (frame history), false = legit (camera pull)
    inline float rcs_x_scale        = 1.0f;
    inline float rcs_y_scale        = 1.0f;
    inline float rcs_max_x          = 5.0f;
    inline float rcs_max_y          = 5.0f;
    inline int   rcs_delay_shots    = 1;

    // ==================== NO SPREAD / NO RECOIL ====================
    inline bool  rage_no_spread     = false;   // zeroes VData spread on rage weapons
    inline bool  legit_no_spread    = false;   // same for legit

    // ==================== ANTI-AIM ====================
    inline bool  aa_enabled     = false;
    inline int   aa_key         = 0;       // 0 = always on when enabled
    inline int   aa_pitch       = 1;       // 0=Off 1=Down 2=Up 3=Zero 4=Minimal
    inline int   aa_yaw         = 1;       // 0=Off 1=Backward 2=Spin 3=Static 4=Jitter
    inline float aa_yaw_base    = 0.f;     // static mode yaw offset
    inline float aa_jitter      = 22.f;    // jitter half-range degrees
    inline float aa_spin_speed  = 6.f;     // degrees per CreateMove tick

    // ==================== TRIGGERBOT ====================
    inline bool triggerbot_enabled = false;
    inline int triggerbot_delay = 0;
    inline int triggerbot_hitbox_filter = 0;   // legacy (Any/Head) — kept for compat
    // Per-hitbox selection: 0=Head 1=Neck 2=Chest 3=Stomach 4=Arms 5=Legs
    inline bool triggerbot_hitboxes[6] = { true, false, false, false, false, false };
    ...
    inline bool  misc_quickstop           = false;  // counter-strafe to a dead stop
    // 0=In Air (kill horiz vel at jump peak), 1=On Land (ground only), 2=Between Shots (always)
    inline int   misc_quickstop_mode      = 1;
    inline bool  misc_autostrafer         = false;
    inline float misc_autostrafer_smooth  = 1.0f;
    inline bool  misc_autostrafer_subtick = true;   // release W in air for free speed boost
    inline bool misc_infinite_duck = false;
    // Auto Pistol: hold LMB and semi-auto pistols (Tec-9, Glock, Deagle…) fire
    // continuously like a rifle instead of one shot per click.
    inline bool misc_auto_pistol = false;
    ...
    // ==================== INDICATOR POSITIONS (draggable overlays) ============
    // {x, y} screen coordinates. Negative x = "use default" (computed at render
    // time from the display size, then stored).
    inline float ind_watermark_pos[2] = { 12.f, 12.f };
    inline float ind_spec_pos[2]      = { 12.f, 64.f };
    inline float ind_keybind_pos[2]   = { -1.f, -1.f };  // default: bottom-right
    inline float ind_dmglog_pos[2]    = { -1.f, -1.f };  // default: top-right

    // ==================== MISC EXTENDED ====================
    inline bool  misc_auto_accept          = false;
    inline bool  misc_show_money           = false;
    // Player Info extras: Armor, Defuse Kit, Zeus, Pistol, Primary
    inline bool  misc_player_info[5]       = { false, false, false, false, false };
    ...
    inline float misc_thirdperson_dist     = 150.f;   // camera distance, 50-300
    inline bool  misc_aspect_ratio_enabled = false;
    inline float misc_aspect_ratio         = 4.f / 3.f;  // 4:3 stretched default
    ...
    inline bool  misc_remove_crosshair     = false;
    inline bool  misc_remove_scope_overlay = false;
    inline bool  misc_remove_legs          = false;   // hide own world model (legs when looking down)
    inline bool  misc_remove_shadows       = false;   // needs m_flShadowStrength offset
    inline bool  misc_remove_team_intro    = false;   // skip "FIRST HALF" team intro scene
    inline bool  misc_hit_marker            = false;
    inline bool  misc_hit_marker_tracer_team  = false; // draw tracers for teammates' shots
    inline bool  misc_hit_marker_tracer_enemy = false; // draw tracers for enemies' shots
    inline int   misc_hit_marker_style     = 0;        // 0 = Classic, 1 = Animated, 2 = Bloom
    inline float misc_hit_marker_color[4]   = { 1.f, 1.f, 1.f, 1.f };
    inline float misc_hit_marker_size       = 5.f;
    inline float misc_hit_marker_duration   = 0.6f;
    inline float misc_smoke_alpha          = 0.f;
